...
{
    public class OracleQueryable<T> : QueryableProvide<T>
    {
        public OracleQueryable(IRepository _Repository)
            : base(_Repository) { }
        ...
        internal static SqlBuilderResult ToSqlStatic(QueryableSqlBuilder query)
        {
            ...
            if (query.Offset.HasValue && query.Limit.HasValue)
            {
                ...
                sb.AppendLine(";");
                //select * from (select T_.*,rownum RowNo_ from (select {0} from {1} {2}) T_) T2_ where T2_.RowNo_ between {3} and {4}
                sb.Append("select * from (select T_.*,rownum RowNo_ from (select ")
                    .Append(selectColumns).Append(" from ").Append(fromSql).Append(whereSql).Append(groupBy)
                    .Append(orderBySql).Append(") T_) T2_ where T2_.RowNo_ between ").Append(query.Offset)
                    .Append(" and ").Append(query.Limit);
            ...
            else
            {
                ...
                {
                    //select {0} from {1} {2} and rownum <= {3} {4}
                    query.Offset = 0;
                    sb.Append("select ").Append(selectColumns).Append(" ").Append(" from ").Append(fromSql)
                        .Append(whereSql).Append(groupBy).Append(" and rownum <= ").Append(query.Limit).Append(" ").Append(orderBySql);
                    result.Sql = sb.ToString();
                }
                else
                {
            ...
            if (query.SelectIntoTable != null)
            {
                ...
                {
                    var columns = classMapper.Properties.Where(i => !i.Ignored);
                    if (!columns.Any())
                    {
    ...
    public class OracleQueryable<T, T2> : QueryableProvide<T, T2>
    {
        public OracleQueryable(IRepository _Repository)
            : base(_Repository) { }

        public override SqlBuilderResult ToSql() => OracleQueryable<T>.ToSqlStatic(new QueryableSqlBuilder
        {
            Function = Function,
            Where = __Where,
            FunctionSql = FunctionSql,
    ...
    public class OracleQueryable<T, T2, T3> : QueryableProvide<T, T2, T3>
    {
        public OracleQueryable(IRepository _Repository)
            : base(_Repository) { }
    ...
    public class OracleQueryable<T, T2, T3, T4> : QueryableProvide<T, T2, T3, T4>
    {
        public OracleQueryable(IRepository _Repository)
            : base(_Repository) { }
    ...
    public class OracleQueryable<T, T2, T3, T4, T5> : QueryableProvide<T, T2, T3, T4, T5>
    {
        public OracleQueryable(IRepository _Repository)
            : base(_Repository) { }
    ...
    public class OracleQueryable<T, T2, T3, T4, T5, T6> : QueryableProvide<T, T2, T3, T4, T5, T6>
    {
        public OracleQueryable(IRepository _Repository)
            : base(_Repository) { }
